...
보조 큐비트를 `|->`로 준비하면, 오라클을 지난 뒤 보조 큐비트는 다시 같은 `|->` 모양으로 돌아온다. 대신 중첩을 이루는 각 `x` 기저 상태 항의 확률 진폭에 다음 부호가 남는다.

- `f(x)=0`인 항: `+` 표식
- `f(x)=1`인 항: `-` 표식

이 `+`와 `-`를 **위상 표식**이라고 부른다. 측정 확률이 양수나 음수라는 뜻이 아니라, 두 항이 마지막에 다시 섞일 때 서로 보강할지 상쇄할지를 정하는 확률 진폭의 부호다.
...
:::expander 이 부호가 첫 큐비트의 위상 표식으로 정리되는 과정을 수식으로 확인하기
...
여기서 $y\oplus f(x)$와 $(-1)^{f(x)}$가 같은 값이라는 뜻은 아니다. 앞의 식은 **보조 큐비트의 값**, 뒤의 식은 잠시 뒤 두 큐비트의 해당 상태 항 전체에 곱해지는 **부호**다.
...
오라클이 첫 큐비트를 직접 뒤집은 것은 아니다. `-1`은 두 큐비트의 해당 상태 항 전체에 곱해진 수다. 보조 큐비트는 다시 같은 `|->` 모양으로 돌아오므로, 이 부호를 첫 큐비트의 해당 기저 상태 항에 남은 상대위상으로 표현할 수 있다.
...
로 바뀌고, **보조 큐비트**는 계속 $\lvert-\rangle_2$로 남는다. 오라클 전 첫 큐비트의 `|+>`는 `+,+`이고, `f(0)=f(1)=0`인 오라클이라면 이 부호가 그대로 유지된다. 오라클 B에서는 `x=1` 항에만 `-`가 붙으므로 `+,-`로 바뀐다. 이것이 본문에서 설명한 위상 표식 변화다.

:::

## 7. 마지막 H가 숨은 표식을 측정값으로 바꾼다

앞 설명에서 필요한 핵심은 하나다. 오라클은 첫 큐비트의 두 기저 상태 항에 `+` 또는 `-` 위상 표식을 남긴다.

- constant 규칙은 `f(0)`과 `f(1)`이 같으므로 두 표식도 같다: `+,+` 또는 `-,-`
- balanced 규칙은 `f(0)`과 `f(1)`이 다르므로 두 표식이 반대다: `+,-` 또는 `-,+`

마지막 H는 이 부호 관계를 구분하는 역할을 한다. 같은 표식은 결과 `0`으로, 반대 표식은 결과 `1`로 모은다. 따라서 최종 측정은 다음처럼 결정된다.
...
따라서 마지막 H 뒤에는 $-\lvert0\rangle$이 된다. 상태 전체에 붙은 `-`는 **전역 위상**이라 측정 확률을 바꾸지 않으므로 결과는 여전히 `0`이다.
